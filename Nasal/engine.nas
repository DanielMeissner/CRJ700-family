...
Engine.Apu = func() {
    ...
    apu.sov_node = props.globals.getNode("/engines/engine[2]/sov", 1);
    apu.sov_node.setValue(0);
...
Engine.Jet = func(n)
{
    ...
    jet.controls = {
        cutoff: 0, fire_ex: 0, 
        reverser_arm: 0, reverser_cmd: 0,
        starter_cmd: 0, thrust_mode: 0, 
        throttle: 0
    };
    ...
    jet.sov_node = props.globals.getNode("/engines/engine["~n~"]/sov", 1);
    jet.sov_node.setValue(jet.sov);
    #EICAS display
    ...
    jet.update = func
    {
        ...
        if (flight_model == "yasim")
        {
            ...
            elsif (jet.serviceable and jet.controls.starter_cmd and jet._has_bleed_air())
            {
                ...
                if (jet.n2 > 30) jet.n1 = math.min(jet.n1 + 1.3 * time_delta, jet.fdm_n1);
